...
const CHANNEL_ENTRYPOINT_PACKAGE = "@tickernelz/paperclip-pro";
...
// Sections whose @tickernelz workspace deps are rewritten to the calver release
...
// release, and every @tickernelz/paperclip-pro-* workspace: dep it declares is rewritten to that
  ...
  for (const pkg of packages) {
    ...
    for (const section of RESOLVED_DEP_SECTIONS) {
      ...
      for (const [depName, spec] of Object.entries(deps)) {
        if (!depName.startsWith("@tickernelz/paperclip-pro-")) continue;
...
function replaceWorkspaceDeps(deps, version) {
  ...
  for (const [name, value] of Object.entries(next)) {
    if (!name.startsWith("@tickernelz/paperclip-pro-")) continue;
