...
packages/adapters/<name>/
  src/
    ...
    cli/
      ...
      format-event.ts   # Colored terminal output for `paperclip-pro run --watch`
  package.json
  tsconfig.json
```
...
## 2. Shared Types (`@tickernelz/paperclip-pro-adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@tickernelz/paperclip-pro-adapter-utils` (types) or `@tickernelz/paperclip-pro-adapter-utils/server-utils` (runtime helpers).
...
```json
{
  "name": "@tickernelz/paperclip-pro-adapter-<name>",
  "version": "0.0.1",
  "private": true,
  "type": "module",
  "exports": {
  ...
  "dependencies": {
    "@tickernelz/paperclip-pro-adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@tickernelz/paperclip-pro-adapter-utils/server-utils`)
...
Pretty-prints stdout lines for `paperclip-pro run --watch`. Use `picocolors` for coloring.
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@tickernelz/paperclip-pro-adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@tickernelz/paperclip-pro-adapter-my-agent";
...
import { parseMyAgentStdoutLine } from "@tickernelz/paperclip-pro-adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@tickernelz/paperclip-pro-adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@tickernelz/paperclip-pro-adapter-my-agent/cli";
...
Import from `@tickernelz/paperclip-pro-adapter-utils/server-utils`:
...
- Package name: `@tickernelz/paperclip-pro-adapter-<kebab-name>`
...
- Default prompt should use `DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE` from `@tickernelz/paperclip-pro-adapter-utils/server-utils` so local adapters share Paperclip's execution contract: act in the same heartbeat, avoid planning-only exits unless requested, leave durable progress and a next action, use child issues instead of polling, mark blockers with owner/action, and respect governance boundaries.
