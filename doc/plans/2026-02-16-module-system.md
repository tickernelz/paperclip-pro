...
- **`id`**: Unique identifier, used as the npm package name suffix (`@tickernelz/paperclip-pro-mod-observability`)
...
```typescript
import type { ModuleAPI } from "@tickernelz/paperclip-pro-core";
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@tickernelz/paperclip-pro-mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@tickernelz/paperclip-pro-mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
...
```bash
pnpm paperclip-pro store list                    # browse available modules and templates
pnpm paperclip-pro store install <module-id>     # install a module
pnpm paperclip-pro store import <template-id>    # import a company template
pnpm paperclip-pro store export                  # export current company as template
```
...
Add to `@tickernelz/paperclip-pro-server`:
...
Add to `@tickernelz/paperclip-pro-ui`:
...
10. **`@tickernelz/paperclip-pro-module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
