...
- Default local path is under instance root: `~/.paperclip-pro/instances/<instanceId>/data/storage`.
...
- `paperclip-pro onboard` writes a valid `storage` config block by default.
- `paperclip-pro configure --section storage` can switch between local and s3 modes.
...
- `paperclip-pro doctor` reports actionable storage status.
