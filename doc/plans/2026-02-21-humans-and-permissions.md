...
- local mode that still feels instant (`npx @tickernelz/paperclip-pro run` and go)
...
2. CLI command `pnpm paperclip-pro auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm paperclip-pro onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm paperclip-pro onboard`).
...
- expose key instance settings in API + CLI (`paperclip-pro configure` / `paperclip-pro onboard`)
...
7. `pnpm paperclip-pro onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
