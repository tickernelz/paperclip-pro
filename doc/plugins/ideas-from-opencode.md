...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@tickernelz/paperclip-pro-plugin-sdk/ui`) with design tokens and common components.
...
A `@tickernelz/paperclip-pro-plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Paperclip instance.
...
- **Single SDK package**: `@tickernelz/paperclip-pro-plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@tickernelz/paperclip-pro-plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
```ts
import { definePlugin, z } from "@tickernelz/paperclip-pro-plugin-sdk";
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@tickernelz/paperclip-pro-plugin-sdk/ui";
...
- plugin UI bundle loading, host bridge, `@tickernelz/paperclip-pro-plugin-sdk/ui`
...
- `@tickernelz/paperclip-pro-plugin-test-harness` and `create-paperclip-plugin` starter template
...
8. Ship `@tickernelz/paperclip-pro-plugin-test-harness` and `create-paperclip-plugin` starter template.
