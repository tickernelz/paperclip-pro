...
- The current runtime ships a small host-provided plugin UI component kit through `@tickernelz/paperclip-pro-plugin-sdk/ui`, but does not support plugin asset uploads/reads yet. Treat plugin asset APIs as future-scope ideas, not current implementation promises.
...
- `~/.paperclip-pro/instances/default/plugins/package.json`
- `~/.paperclip-pro/instances/default/plugins/node_modules/`
- `~/.paperclip-pro/instances/default/plugins/.cache/`
- `~/.paperclip-pro/instances/default/data/plugins/<plugin-id>/`
...
- `npx @tickernelz/paperclip-pro plugin install <package[@version]>`
- `npx @tickernelz/paperclip-pro plugin uninstall <plugin-id>`
- `npx @tickernelz/paperclip-pro plugin upgrade <plugin-id> [version]`
- `npx @tickernelz/paperclip-pro plugin doctor <plugin-id>`
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@tickernelz/paperclip-pro-plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@tickernelz/paperclip-pro-plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@tickernelz/paperclip-pro-plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@tickernelz/paperclip-pro-plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@tickernelz/paperclip-pro-plugin-sdk/ui`
...
The `@tickernelz/paperclip-pro-plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
6. The operator may force-purge immediately via CLI: `npx @tickernelz/paperclip-pro plugin purge <plugin-id>`.
...
### 27.1 `@tickernelz/paperclip-pro-plugin-test-harness`
...
```ts
import { createTestHarness } from "@tickernelz/paperclip-pro-plugin-test-harness";
...
- The operator installs the plugin from a local path: `npx @tickernelz/paperclip-pro plugin install ./path/to/plugin`
...
- `@tickernelz/paperclip-pro-plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@tickernelz/paperclip-pro-plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@tickernelz/paperclip-pro-plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@tickernelz/paperclip-pro-plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@tickernelz/paperclip-pro-plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@tickernelz/paperclip-pro-plugin-sdk/ui`
...
- `@tickernelz/paperclip-pro-plugin-test-harness`
