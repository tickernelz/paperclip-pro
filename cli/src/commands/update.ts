...
async function runPreUpdateBackup(options: UpdateOptions, backup: () => Promise<void>, hasInstanceData = hasPaperclipInstanceData): Promise<void> {
  ...
  try {
    ...
    if (isDatabaseUnreachableError(error)) {
      throw new Error(
        "The Paperclip database is not running or reachable, so the pre-update backup cannot be taken. Start the service with `paperclip-pro service start` and retry, or skip the backup with `paperclip-pro update --no-backup`.",
        { cause: error },
      );
    }
    throw error;
...
export function detectInstallMode(executablePath = process.argv[1] ?? "", paths = resolveInstallStorePaths()): InstallMode {
  ...
  if (normalized.includes("/node_modules/@tickernelz/paperclip-pro/")) return "global-npm";
...
export async function updateCommand(options: UpdateOptions, overrides: Partial<Dependencies> = {}): Promise<void> {
  ...
  if (options.rollback) {
    ...
    emit(options, { mode, action: "rollback", version: next.version, restarted }, pc.green(`Rolled back to paperclip-pro ${next.version}${restarted ? " and restarted the active service" : ""}. Database migrations are not reversed; restore the pre-update backup if needed.`));
    return;
  }
  if (mode === "npx") { emit(options, { mode, action: "install" }, "This is an ephemeral npx install. Run `paperclip-pro install`, then use `paperclip-pro update` from the managed shim."); return; }
  ...
  if (options.check) { emit(options, { mode, currentVersion: currentVersion ?? null, targetVersion, updateAvailable: comparison > 0, downgrade: comparison < 0, channel: request.channel }, comparison > 0 ? `Update available: ${targetVersion}` : comparison < 0 ? `Target ${targetVersion} is older than ${currentVersion}.` : `paperclip-pro ${targetVersion} is current.`); if (comparison > 0) process.exitCode = 10; return; }
  if (mode === "global-npm") {
    ...
    const args = ["install", "-g", `@tickernelz/paperclip-pro@${targetVersion}`, `--registry=${PUBLIC_NPM_REGISTRY}`, `--@tickernelz:registry=${PUBLIC_NPM_REGISTRY}`]; console.log(`Running: npm ${args.join(" ")}`);
    if (!options.dryRun) {
  ...
  emit(options, { mode, currentVersion, targetVersion, changed: true, reused: installed.reused, restarted }, pc.green(`Updated paperclip-pro ${currentVersion} → ${targetVersion}${restarted ? " and restarted the active service" : ""}. Run \`paperclip-pro update --rollback\` for an instant payload rollback.`));
}
