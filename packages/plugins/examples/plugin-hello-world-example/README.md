# @tickernelz/paperclip-pro-plugin-hello-world-example
...
```bash
pnpm --filter @tickernelz/paperclip-pro-plugin-hello-world-example build
npx @tickernelz/paperclip-pro plugin install ./packages/plugins/examples/plugin-hello-world-example
```
...
- **Reinstall after pulling.** If you installed a plugin by local path before the server stored `package_path`, the plugin may show status **error** (worker not found). Uninstall and install again so the server persists the path and can activate the plugin:  
  `npx @tickernelz/paperclip-pro plugin uninstall paperclip.hello-world-example --force` then  
  `npx @tickernelz/paperclip-pro plugin install ./packages/plugins/examples/plugin-hello-world-example`.
