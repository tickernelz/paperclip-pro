...
- **Worker SDK:** `@tickernelz/paperclip-pro-plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@tickernelz/paperclip-pro-plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@tickernelz/paperclip-pro-plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@tickernelz/paperclip-pro-plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@tickernelz/paperclip-pro-plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@tickernelz/paperclip-pro-plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@tickernelz/paperclip-pro-plugin-sdk/ui` | UI entry: hooks, host navigation, HTTP-safe clipboard copy, shared components, and slot prop types |
| `@tickernelz/paperclip-pro-plugin-sdk/ui/hooks` | Hooks only |
| `@tickernelz/paperclip-pro-plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@tickernelz/paperclip-pro-plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@tickernelz/paperclip-pro-plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@tickernelz/paperclip-pro-plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@tickernelz/paperclip-pro-plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@tickernelz/paperclip-pro-plugin-sdk/types` | Worker context and API types (advanced) |
...
```bash
pnpm add @tickernelz/paperclip-pro-plugin-sdk
```
...
- The host ships a small shared React component kit through `@tickernelz/paperclip-pro-plugin-sdk/ui`. Use it for native Paperclip controls; custom React and CSS are still supported.
...
```ts
import { definePlugin, runWorker } from "@tickernelz/paperclip-pro-plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@tickernelz/paperclip-pro-plugin-sdk`.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@tickernelz/paperclip-pro-plugin-sdk`.
...
```tsx
import { usePluginData, usePluginAction } from "@tickernelz/paperclip-pro-plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@tickernelz/paperclip-pro-plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@tickernelz/paperclip-pro-plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@tickernelz/paperclip-pro-plugin-sdk/ui";
...
```tsx
import { useHostNavigation } from "@tickernelz/paperclip-pro-plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@tickernelz/paperclip-pro-plugin-sdk/ui";
...
The host provides selected shared UI components through `@tickernelz/paperclip-pro-plugin-sdk/ui`.
...
```tsx
import { copyTextToClipboard } from "@tickernelz/paperclip-pro-plugin-sdk/ui";
...
```tsx
import { MarkdownBlock, MarkdownEditor } from "@tickernelz/paperclip-pro-plugin-sdk/ui";
...
```tsx
import { FileTree, type FileTreeNode } from "@tickernelz/paperclip-pro-plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@tickernelz/paperclip-pro-plugin-sdk/ui`.
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@tickernelz/paperclip-pro-plugin-sdk/ui";
...
```ts
import { createPluginBundlerPresets } from "@tickernelz/paperclip-pro-plugin-sdk/bundlers";
...
```ts
import { startPluginDevServer } from "@tickernelz/paperclip-pro-plugin-sdk/dev-server";
