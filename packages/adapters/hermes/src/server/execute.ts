...
const HERMES_DEFAULT_PROMPT_TEMPLATE = [
  ...
  "- Do every Paperclip read and write through the Paperclip MCP tools; terminal and browser tools do not reach the board.",
  "- Do not copy secrets into comments or commands; the tools carry the run credential for you.",
  ...
  "",
  "Multiline update pattern: pass the Markdown straight through as a tool argument.",
  "`paperclipUpdateIssue` with `id: \"{{context.issueId}}\"`, `status: \"done\"`, and `comment` set to the full multiline body.",
  "",
  DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE,
].join("\n");
